      ...
      <aside className="hidden md:flex md:w-64 md:flex-col md:border-r md:border-cream-200 md:bg-white">
        <div className="px-5 py-5">
          ...
          {activeEvent ? (
            ...
            >
              <div className="text-[11px] uppercase tracking-wide text-matcha-700">Active event</div>
              <div className="mt-0.5 text-sm font-semibold text-matcha-900">{activeEvent.name}</div>
              <div className="mt-1 flex items-center justify-between text-xs text-matcha-900/70">
                <span>{totals?.totalOrders ?? 0} orders</span>
                <span>{formatMoney(totals?.revenuePaid ?? 0)}</span>
              </div>
            </button>
          ) : null}
        </div>
        ...
        {activeEvent ? (
          ...
          >
            ...
            <span className="opacity-70">
              {totals?.totalOrders ?? 0} orders · {formatMoney(totals?.revenuePaid ?? 0)}
            </span>
          </button>
        ) : null}
