...
function Section({
  ...
  return (
    <section>
      <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-matcha-900/60">
        {title}
      </h2>
      {children}
    </section>
    ...
    <Card className={isArchived ? "opacity-70" : undefined}>
      <div className="flex flex-wrap items-center gap-3">
        <div className="min-w-0 flex-1">
          ...
          {item.description ? (
            <div className="mt-1 text-xs text-matcha-900/60">{item.description}</div>
          ) : null}
        </div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="text-right">
      <div className="text-[10px] uppercase tracking-wide text-matcha-900/50">
        {label}
      </div>
      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-matcha-900/60">
    ...
    <tr className="border-t border-cream-100">
      ...
      <td className="py-1.5 pr-3">
        <Select
          ...
          value={ing.pool ?? ""}
          onChange={(e) =>
            onChange({ pool: (e.target.value || null) as Ingredient["pool"] })
          }
        >
          <option value="">—</option>
...
function NewIngredientForm({
  ...
  const [pool, setPool] = useState<"" | "milk" | "cream">("");
  ...
  return (
    <div className="space-y-3">
      ...
      <Field label="Pool (optional)">
        <Select value={pool} onChange={(e) => setPool(e.target.value as "" | "milk" | "cream")}>
