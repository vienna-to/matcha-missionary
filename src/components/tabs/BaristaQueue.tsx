...
export default function BaristaQueue() {
  ...
  useEffect(() => {
    ...
    if (fresh.length > 0) {
      if (state.settings.baristaPingEnabled) playPing();
      setFreshIds((s) => {
  ...
  }, [allOrders, state.settings.baristaPingEnabled]);
  ...
  return (
    <div className="space-y-5">
      <header>
        <h1 className="text-xl font-semibold">Barista Queue</h1>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
            <div className={cn("font-semibold", compact ? "text-base" : "text-2xl")}>#{order.orderNumber}</div>
            <div className={cn("truncate", compact ? "text-sm" : "text-lg")}>{order.customerName}</div>
