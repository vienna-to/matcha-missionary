    ...
    <div className="lg:grid lg:grid-cols-[1fr_360px] lg:gap-6">
      <div>
        <header className="mb-4">
          ...
          <p className="mt-0.5 text-sm text-matcha-900/60">
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
      {items.map((item) => (
        <button
          key={item.id}
          onClick={() => onAdd(item)}
          className="group relative flex flex-col rounded-2xl border border-cream-200 bg-white p-4 text-left transition-all hover:border-matcha-300 hover:shadow-md active:scale-[0.98]"
        >
          ...
          {item.description ? (
            <div className="mt-2 line-clamp-2 text-xs text-matcha-900/60">
              {item.description}
            </div>
          ) : null}
          <div className="mt-3 flex items-center justify-between">
            <div className="text-[11px] text-matcha-900/50">
              {item.allowedMilkIds.length > 0 ? "Milk" : ""}
              {item.allowedMilkIds.length > 0 && item.allowedCreamIds.length > 0 ? " · " : ""}
              {item.allowedCreamIds.length > 0 ? "Cream" : ""}
            </div>
    ...
    <Card className="space-y-4 p-4">
        ...
        <Field label="Payment status">
          <div className="flex gap-1.5">
            {(["paid", "unpaid", "comped"] as PaymentStatus[]).map((s) => (
              ...
              >
                {s === "paid" ? "Paid" : s === "unpaid" ? "Unpaid" : "Comped"}
              </Chip>
            ))}
          </div>
        </Field>
      ...
      <div className="rounded-xl bg-matcha-50 p-3">
        <div className="flex items-end justify-between">
          <div>
            <div className="text-[11px] uppercase tracking-wide text-matcha-700">Total</div>
    ...
    <div className="rounded-xl border border-cream-200 bg-cream-50/50 p-2.5">
      <div className="flex items-start justify-between gap-2">
        <button onClick={onEdit} className="min-w-0 flex-1 text-left">
          ...
          {mods.length > 0 ? (
            <div className="mt-0.5 line-clamp-2 text-xs text-matcha-900/60">
              {mods.join(" · ")}
            </div>
          ) : (
            <div className="mt-0.5 text-xs italic text-matcha-900/40">
              tap to customize
            </div>
          )}
        </button>
