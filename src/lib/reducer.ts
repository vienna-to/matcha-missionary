  ...
  switch (action.type) {
    ...
    case "CREATE_EVENT": {
      const snapshot: MenuSnapshot = {
        id: newId("snap"),
        menuItems: state.menuItems.filter((m) => m.active).map((m) => ({ ...m })),
      ...
      const event: Event = {
        ...
        isActive: state.events.length === 0, // first event auto-active
        createdAt: nowIso(),
        updatedAt: nowIso(),
      };
      return {
    ...
    case "UPDATE_ORDER":
      return {
        ...
        orders: state.orders.map((o) =>
          o.id === action.id ? { ...o, ...action.patch, updatedAt: nowIso() } : o,
        ),
      };
        ...
        orders: state.orders.map((o) => {
          ...
          const status: OrderStatus =
            o.status === "cancelled"
              ? "cancelled"
              : derived ?? o.status;
          return { ...o, items, status, updatedAt: nowIso() };
        }),
      };
